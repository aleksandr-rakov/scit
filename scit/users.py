...
class UsersViews(api.BaseViews):
    ...
    @api.view(path='users/{_id}', method='GET')
    def view_get(self):
    ...
    @api.view(path='users/{_id}', method='POST')
    ...
    @api.view(path='users/{_id}/change_password', method='POST')
    def view_change_pasword(self):
        _id=ObjectId(self.params['_id'])
        item=self.db[_collection].find_one({'_id': _id})
        if item is None:
            raise HTTPNotFound()
